...
# Summary: Grading application that displays lowest scores, pecentage of the class that fails,  and who recieved the lowest scores
...
public class assignment4 {
   ...
   // Boolean counter;
    ...
    List<String> failedStudentsNames = new ArrayList<>();
...
for(int x: studentScore){
    if(x<50){
        failedStudents.add(x);
        failedStudentsNames.add(studentName.get(index));
    }
    index += 1;
}
    double numberofStudents = studentName.size(); 
    double failedTest = failedStudents.size();
    double percentageFailed = 100*(failedTest/numberofStudents);

    System.out.println();
    System.out.println("Total of scores entered: " + numberofStudents);
    System.out.println("Total number of students that failed the failed the exam: " + Math.round(failedTest));
    System.out.println("Percentage of class that failed the exam: " + percentageFailed );
    System.out.print(failedStudentsNames.get(failedStudents.indexOf(Collections.min(failedStudents))) + " received the lowest score:" + Collections.min(failedStudents));
